                ...
                <TableBody>
                    {employment && employment.map(employ =>(
                        <TableRow key={employ.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                            ...
                            {employ.status =="ВУЗ" && <TableCell><p className={s.study}>{employ.status}</p></TableCell>}
                            {employ.status =="Срочная служба" && <TableCell><p className={s.study}>{employ.status}</p></TableCell>}
                            {employ.status =="ИП" && <TableCell><p className={s.work}>{employ.status}</p></TableCell>}
                            {employ.status =="Трудоустроен" && <TableCell ><p className={s.work}>{employ.status}</p></TableCell>}
                            {employ.status =="Не трудоустроен" && <TableCell ><p className={s.dontWork}>{employ.status}</p></TableCell>}
                            {employ.status =="Работает не официально" && <TableCell ><p className={s.dontWork}>{employ.status}</p></TableCell>}
                            {employ.status =="Самозанятый" && <TableCell ><p className={s.work}>{employ.status}</p></TableCell>}
