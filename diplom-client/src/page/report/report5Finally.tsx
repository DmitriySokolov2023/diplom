...
export const ReportPage5Finally = (props:any) => {
    ...
    const {data: status, refetch} = useQuery({queryKey: ['statusInEmployment'], queryFn:  () => reportApi.getByGruppa<IReportStatus[]>({students:stId})})
                ...
                <Table sx={{minWidth: 650}} aria-label="simple table">
                    <TableHead>
                        <TableRow>
                            ...
                            {status?.length > 0 &&
                                <TableCell>
                                    <CSVLink data={report} separator={";"} filename={`Отчет_ТУ_Выпускники года: ${props.ye} | ${newDate}`}>
                                        <SimCardDownloadIcon/>
                                    </CSVLink>
                                </TableCell>
                            }
                ...
                <>
                    ...
                    <TableContainer component={Paper}>
                        <Table sx={{minWidth: 650}} aria-label="simple table">
                            ...
                            <TableBody>
                                <TableRow>
                                    <TableCell><span className={s.strong}>Учебная группа: </span> {props.title}</TableCell>
                                    <TableCell></TableCell>
                                </TableRow>

                                <TableRow>
                                    <TableCell><span className={s.strong}>Год выпуска: {props.ye}</span></TableCell>
                                    <TableCell></TableCell>
                                </TableRow>
                                <TableRow>
