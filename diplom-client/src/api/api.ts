...
export class Api{
    constructor(private path:string, private baseUrl = 'http://localhost:3200/') {}
...
export const groupApi = {
    ...
    async getGroup<T>():Promise<T>{
        const group = api.get(this.path)
        return await group
    },
    async getGroupIn<T>(obj:object):Promise<T>{
        const group = api.get(this.path + 'gr', obj)
        return await group
    },
...
export const reportApi = {
    ...
    async getByStatus<T>(obj:object):Promise<T>{
        return await api.get(this.path + `stat`, obj)
    },
    async getByStatusGruppa<T>(obj:object):Promise<T>{
        return await api.get(this.path + `status`, obj)
    },
    async getByGruppa<T>(obj:object):Promise<T>{
        return await api.get(this.path + `students`, obj)
    },
    async getAllEmploy<T>():Promise<T>{
        return await api.get(this.path)
    },
}
